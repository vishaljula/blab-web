...
// Step counts per path (path-choice is now its own step)
export const SELF_TOTAL_STEPS = 5;    // Type → PathChoice → Location → Details+Price → Review
export const REALTOR_TOTAL_STEPS = 4; // Type → PathChoice → Location → RealtorList
...
> = {
  ...
  currentStep: 1,
  totalSteps: SELF_TOTAL_STEPS,  // updated when path chosen
};
